# Changelog

## [Unreleased]

### Added
- Shift+E: Effect Search popup (Control module)
- Close button [x] on Effect Search panel
- ESC key closes Effect Search panel
- CLAUDE.md: Model usage guidelines

### Fixed
- Install path: MediaCore folder (not After Effects folder)
- CI: Force clean build with --clean-first

---

## [1.0.0] - 2025-12-16

### Added
- Y key (0.4s hold): Anchor Grid popup
- Grid size: 3x3 to 7x7 configurable
- Custom anchor points (3 slots)
- Copy/Paste anchor
- Composition mode / Selection mode toggle
- Mask recognition mode
- CEP Settings panel
- Double-tap Y: Toggle mode (click to apply)

### Install Paths
- Plugin: `MediaCore/AnchorSnap.aex`
- CEP: `com.anchor.snap`
