...
### 구현 노트
- **1단계**: FindEffectControlsWindow()와 CommandHook() 삭제 완료
  - 두 함수 모두 실제로 호출되지 않음을 확인
  - NOTE 주석으로 삭제 이유 문서화
- **2단계**: 키 입력 감지 시스템 문서화
  - KEY INPUT DETECTION SYSTEM 다이어그램 추가
  - 각 함수에 AEGP API 참조 주석 추가
- **3단계**: docs/API_MAPPING.md 생성
  - Windows API vs AEGP API 대체 가능성 표 작성
  - CLAUDE.md에 API Usage 섹션 추가

### 발생한 블로커
- 없음

### 향후 계획을 위한 개선점
- 4단계(macOS 포팅)는 필요 시 진행
- 현재 Windows 환경에서 안정적으로 작동 중
